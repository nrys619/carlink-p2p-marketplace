...
function fallbackAnalysis(mode) {
  ...
  return {
    ...
    options: ['サンルーフ', 'ETC', 'アダプティブクルーズ', 'シートヒーター', '衝突被害軽減ブレーキ', '電動リアゲート'],
    price: 3180000,
    confidence: 0.72,
    source: 'fallback',
...
async function analyzeWithOpenAI({ image, images, mode }) {
  ...
  const prompt =
    mode === 'certificate'
      ? '日本の自動車検査証画像をOCRしてください。読み取れる範囲で、車名、型式、グレード候補、車台番号、初度登録、車検満了、走行距離、排気量、燃料、用途、自家用/事業用、乗車定員、車両重量を推定し、出品者が確認しやすいJSONだけで返してください。'
      : '中古車出品用の車両写真を解析してください。外装、内装、メーター、装備スイッチから、車種、型式候補、グレード候補、年式候補、走行距離、燃料、駆動方式、ミッション、装備候補、状態メモ、推奨価格を推定し、JSONだけで返してください。'
...
返却形式:
{
  "fields": {"車名": "", "型式": "", "グレード": "", "車台番号": "", "初度登録": "", "車検満了": "", "走行距離": "", "排気量": "", "燃料": "", "駆動方式": "", "ミッション": "", "用途": "", "自家用/事業用": "", "乗車定員": "", "車両重量": ""},
  "options": ["ETC", "サンルーフ", "アダプティブクルーズ"],
  "price": 0,
  "confidence": 0.0,
  "note": "確認が必要な点"
}`,
